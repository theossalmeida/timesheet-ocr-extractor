...
For an update, run `deploy\update-all.bat` as administrator: it refuses to run with uncommitted changes to tracked files, fast-forwards the current branch, reinstalls with the tunnel exactly as configured today, and fails loudly if the backend does not answer `/health` afterwards. To do it by hand instead, pull/review the intended Git revision and run the installer again. For a backend-only change there is `deploy\redeploy-soft.bat`: it pulls, swaps the Python code into the release that is already installed and restarts only the backend, reusing the existing virtualenv. It installs dependencies only when `requirements.txt` changed, refuses to run when the revision touches `frontend/` (that needs the Next.js build, so use `update-all.bat`), and accepts `-DryRun` to print what it would do without stopping anything. It records the revision it leaves running in `SOFT-REVISION.txt` inside the release, since the release folder keeps the name of the revision it was built from. A soft redeploy edits the active release in place and does not remove files deleted upstream; `update-all.bat` builds a clean release and is the one to use when in doubt. Releases are kept in `C:\ProgramData\Autus\releases`. Private configuration stays in `config`; changing it requires updating the active backend `.env` and restarting the backend. Current service XML records the actual release paths. Save those XML files before an update. For rollback, stop the AUTUS services, restore the previous service XML and restart; review database compatibility first. Never restore an old unauthenticated release as a public production service.
