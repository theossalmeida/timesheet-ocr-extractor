...
export async function extractTimesheet(
  ...
  try {
    response = await fetch(`${API_URL}/extract/stream`, {
      method: "POST",
      body: form,
      // No client-side timeout. Backend streams keep-alives while OCR runs.
    });
  } catch {
    ...
    for (const part of parts) {
      ...
      if (event.type === "progress") {
        ...
        return {
          ...
          provider: (event.provider as string) ?? "desconhecido",
        };
      } else if (event.type === "error") {
        throw new ApiError(
          (event.message as string) ?? "Erro ao processar o PDF.",
          (event.status as number) ?? 422,
        );
      }
    }
  }
