...
def _build_headers(pair_count: int) -> tuple[list[str], list[int]]:
    ...
    for i in range(1, pair_count + 1):
        ...
        widths += [10, 10]
    headers += ["Intervalo", "Ocorrência", "Tipo", "Aviso OCR"]
    widths += [12, 22, 20, 42]
...
def build_guia_excel(rows: list[TimesheetRow]) -> bytes:
    ...
    guia_headers = ["Data", "Entrada 1", "Saída 1", "Intervalo", "Aviso OCR"]
    guia_widths = [12, 10, 10, 12, 42]
    ...
    for i, row in enumerate(rows, start=1):
        ...
        values = [row.data, marks[0], marks[1], row.intervalo or "", row.ocr_warning or ""]
        alignments = [_LEFT, _CENTER, _CENTER, _CENTER, _LEFT]
    ...
    for i, row in enumerate(result.rows, start=1):
        ...
        values = [
            ...
            *_padded_marcacoes(row, pair_count),
            row.intervalo or "",  # <--- Intervalo logo após as entradas/saídas
            row.ocorrencia_raw,
            TIPO_LABELS.get(row.ocorrencia_tipo or "", "") if row.ocorrencia_tipo else "",
            row.ocr_warning or "",
        ]
        alignments = [_LEFT, *([_CENTER] * (pair_count * 2)), _CENTER, _LEFT, _LEFT, _LEFT]
        for col, (val, align) in enumerate(zip(values, alignments), start=1):
